    ...
    public partial class CustomerCartForm : Form
    {
        ...
        private void orderButton_Click(object sender, EventArgs e)
        {
            if (cartListBox.Items.Count > 0)
            {
                ...
                {
                    order.AppendLine($"Продукт: {product.Name}\nЦіна: {product.Price:C}");
                    product.Available -= product.Reserved;
                    product.Reserved = 0;
                }

                MessageBox.Show($"Деталі покупки:\n\n{order.ToString()}\nВсього: {TotalPrice:C}", "Деталі покупки");
        ...
        private void UpdatePriceLabel() => priceLabel.Text = $"{TotalPrice:C}";
    }
}